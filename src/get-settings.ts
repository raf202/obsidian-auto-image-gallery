import { normalizePath, parseYaml, Platform, App } from "obsidian";
...
const getSettings = (app: App, src: string, container: HTMLElement) => {
  ...
  console.log(app);
  // const app = new App();
  const notePath = app.workspace.getActiveFile().parent.path;
  const noteName = app.workspace.getActiveFile().basename;

  settings.path = normalizePath(
    settingsSrc.path ?? `${notePath}/attachments/${noteName}`
  );
